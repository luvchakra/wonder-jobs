...
export default function EditSchedulePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader back={{ href: "/app/automation/scheduled", label: "Scheduled searches" }} title={schedule.name} description={`Version ${workflow.version} · created ${new Date(schedule.createdAt).toLocaleDateString("en-IN")}`} />
