...
export type ResumeSection =
  ...
  | { type: "skills"; groups: ResumeSkillGroup[] }
...
const GROUPS: [string, RegExp][] = [
  ["Languages", /^(python|java|javascript|typescript|go|golang|rust|c\+\+|c#|c|ruby|php|kotlin|swift|scala|sql|r|bash|shell|powershell|perl|dart|elixir)$/i],
  ["Cloud", /\b(aws|azure|gcp|google cloud|cloud|kubernetes|k8s|docker|terraform|serverless|lambda|openshift)\b/i],
  ["Security", /\b(security|iam|identity|sailpoint|saviynt|cyberark|okta|oracle iam|siem|soc|zero trust|pam|sso|oauth|pki|firewall|vulnerability|compliance|grc|iso 27001|nist)\b/i],
  ["Frameworks", /\b(react|next\.?js|angular|vue|django|flask|spring|rails|node\.?js|express|\.net|fastapi|laravel|pytorch|tensorflow)\b/i],
  ["Data", /\b(data|analytics|sql|spark|hadoop|kafka|snowflake|bigquery|tableau|power bi|looker|dbt|etl|machine learning|ml|ai|statistics)\b/i],
...
export function buildResumeDocument(dna: CareerDNA, opts: { target?: TargetJob; now?: string } = {}): ResumeDocument {
  ...
  const strengths = dna.strengths.map(clean).filter(Boolean);
  if (strengths.length) sections.push({ type: "strengths", items: strengths });
  if (skills.length) sections.push({ type: "skills", groups: groupSkills(skills) });

  const experience = sortExperience(h.experience.filter((e) => clean(e.employer) && clean(e.title))).map((e) => {
    const bullets = e.bullets.filter((b) => clean(b.text)).map((b) => ({ text: clean(b.text), evidenceIds: [b.id], provenance: b.provenance }) satisfies ResumeBullet);
  ...
  if (experience.length) {
    ...
    const picked = (opts.target ? byRelevance(all, (b) => b.text, target) : all).slice(0, 5);
  ...
  const projects = h.projects.filter((p) => clean(p.name)).map((p) => ({ id: p.id, name: clean(p.name), description: clean(p.description) || undefined, technologies: p.technologies?.map(clean).filter(Boolean), url: clean(p.url) || undefined, bullets: p.bullets?.filter((b) => clean(b.text)).map((b) => ({ text: clean(b.text), evidenceIds: [b.id], provenance: b.provenance })) }));
