...
- **Persistence**: wired to Supabase (project `wonder-ark`, migration `wonderjobs_persistence`). Server-side only: `server/supabase.ts` (service role), `server/state.ts` (per-tenant JSON document per client store → `wonderjobs.app_state`; all tables in the dedicated `wonderjobs` schema, exposed to the service role only), `SupabaseSecretStore` (`ai_provider_secrets`, ciphertext only), `action_audit` for external actions. The browser syncs every Zustand store through `store/remoteStorage.ts` (debounced PUTs, keepalive flush, localStorage cache/fallback). Requires `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `WONDER_SECRET_KEY` in the deployment; without them the app runs local-only and says so on the Profile page. Not yet exercised against the live project from this environment (no service key available here) — the memory-backed code path and the storage adapter are tested.
