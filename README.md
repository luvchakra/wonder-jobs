...
| `WONDERJOBS_AI_KEY` / `WONDERJOBS_AI_MODEL` / `WONDERJOBS_AI_PROVIDER` | Optional. API key (and model) that powers "WonderJobs AI" drafting for every account on the platform's bill. `WONDERJOBS_AI_PROVIDER` picks the vendor — `anthropic` (default, model `claude-sonnet-5`), `openai` (`gpt-5-mini`), or `gemini` (`gemini-2.5-flash`); `WONDERJOBS_AI_MODEL` overrides the default model for whichever vendor is chosen. Without a key for the chosen vendor, drafts are deterministic templates and the AI settings page says so; users can still connect their own key to any of the three. |
